...
function openDetail(week, day) {
  ...
  $("toggleDone").addEventListener("click", () => {
    ...
    if (cur.done) {
      celebrate();
      toast(!isRaceDetail ? DONE[Math.floor(Math.random() * DONE.length)] : w.finish ? "🏁 25 km gelopen! Wat een prestatie, strijder!" : w.race ? "🏅 Finisher! Wat een prestatie, strijder!" : w.tuneup ? "🏁 Wedstrijd voltooid, sterk gepacet!" : DONE[Math.floor(Math.random() * DONE.length)]);
    }
    closeDetail();
  });
